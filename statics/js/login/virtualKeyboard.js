...
(function() {
  ...
  VKI_attach = function(elem,button) {
    if (elem.getAttribute("VKI_attached")) return false;
    if (button) {
          button.alt = self.VKI_i18n['01'];
          button.title = self.VKI_i18n['01'];
          button.elem = elem;
    ...
    for (var x = 0, elem; elem = inputElems[x++];)
      for (var y = 0, ex; ex = elem[y++];){
        if (ex.nodeName == "TEXTAREA" || ex.type == "text" || ex.type == "password"){
			...
			{ 
				var button = ex.parentNode.getElementsByClassName("keyboardButton");
				if(button.length>0)
					VKI_attach(ex,button[0]);
				else
					VKI_attach(ex);
			}
		}
